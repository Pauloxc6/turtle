document.addEventListener("DOMContentLoaded", function () {
    ...
    const consent = localStorage.getItem("turtle_cookie_consent");
    ...
    function loadAnalytics() {
        ...
        gtag('js', new Date());
        gtag('config', 'G-QBVQE7WJNW');

        // Injeta a tag script assíncrona do GA4
        const scriptGtag = document.createElement("script");
        scriptGtag.async = true;
        scriptGtag.src = "https://www.googletagmanager.com/gtag/js?id=G-QBVQE7WJNW";
    ...
    if (btnAccept) {
        btnAccept.addEventListener("click", function () {
            localStorage.setItem("turtle_cookie_consent", "accepted");
    ...
    if (btnReject) {
        btnReject.addEventListener("click", function () {
            localStorage.setItem("turtle_cookie_consent", "rejected");
            if (banner) banner.style.display = "none";
        });
    }
    ...
    if (btnReset) {
        btnReset.addEventListener("click", function () {
            localStorage.removeItem("turtle_cookie_consent");
